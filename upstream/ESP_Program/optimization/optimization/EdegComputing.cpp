...
Computer::Computer(short int FFT_N,short int Quantity_of_Axis ):m_FFT_N(FFT_N),axis_num(Quantity_of_Axis){
 //建構函式
}
...
float* Computer::Mean_2D(float** Time_Array,float *mean_2d){
  ...
  for(int i=0;i<axis_num;i++){
    for(int j =0;j<m_FFT_N;j++){
      time_total[axis_num] = time_total[axis_num]+Time_Array[j];  
      }
...
float* Computer::Std_2D(float** Time_Array,float* avg,float* std_2d){
  ...
  for(int i=0;i<axis_num;i++){
    for(int j=0;j<m_FFT_N;j++){
      num_Std[axis_num] = num_Std[axis_num] + pow(Time_Array[j]-avg,2);
    }
...
float* Computer::RMS_2D(float** Time_Array,float* rms_2d){
  ...
  for(int i=0;i<axis_num;i++){
    for(int j=0;j<m_FFT_N;j++){
      MeanSqure[axis_num] = MeanSqure[axis_num]+ pow(Time_Array[j],2);
    }
...
float* Computer::Kurtosis_2D(float** Time_Array,float* avg,float* std,float* kurtosis_2d){
  float std_Kur_pow4[axis_num] = pow(std,4)*m_FFT_N;
